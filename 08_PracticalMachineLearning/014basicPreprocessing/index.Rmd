...
sd(training$capitalAve)  ### skewed and highly variable
```
...
testCapAveS <- (testCapAve  - mean(trainCapAve))/sd(trainCapAve) ### when apply to
     ### test set, have to use train mean and train SD
mean(testCapAveS)  ## won't be exactly 0
sd(testCapAveS)  ## won't be exactly 1
```
...
preObj <- preProcess(training[,-58],method=c("center","scale"))  ### center and scale
...
testCapAveS <- predict(preObj,testing[,-58])$capitalAve  ## applies pre-processing to      ## test set
mean(testCapAveS)
sd(testCapAveS)
```
...
### Box-Cox transformations try to make continuous data look like normal data
preObj <- preProcess(training[,-58],method=c("BoxCox"))
trainCapAveS <- predict(preObj,training[,-58])$capitalAve
par(mfrow=c(1,2))
hist(trainCapAveS)  ### better, still lots 0's
qqnorm(trainCapAveS)  ### better, still not straight line
```
