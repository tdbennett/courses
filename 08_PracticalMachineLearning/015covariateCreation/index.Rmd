...
dummies <- dummyVars(wage ~ jobclass,data=training)   ## strange way to do this
...
nsv <- nearZeroVar(training,saveMetrics=TRUE)   ### this is kind like info content?
nsv
```
...
bsBasis  ### roughly a polynomial of degree 1,2,3 in the columns
```
...
```{r ,dependson="splines",fig.height=4,fig.width=4}
predict(bsBasis,age=testing$age)   ### use the spline basis built on training set
```
