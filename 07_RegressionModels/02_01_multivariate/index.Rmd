...
ey = resid(lm(y ~ x2 + x3))  ## residual for y
ex = resid(lm(x ~ x2 + x3))  ## resid for x on x2 and x3
sum(ey * ex) / sum(ex ^ 2)  ## regression through the origin estimate for ey ~ ex
    ## is the same as the x coefficient in the full model y ~ x + x2 + x3
coef(lm(ey ~ ex - 1))
coef(lm(y ~ x + x2 + x3)) 
```
