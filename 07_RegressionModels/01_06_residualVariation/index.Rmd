...
fit <- lm(y ~ x)
e <- resid(fit)   ## easy way to get residuals
yhat <- predict(fit)
e2 <- y - yhat ### manual residuals
max(abs(e -(y - yhat))) 
...
abline(fit, lwd = 2)
for (i in 1 : n) 
  lines(c(x[i], x[i]), c(y[i], yhat[i]), col = "red" , lwd = 2)  ### adds little red lines
```
...
## Non-linear data
```{r, echo = FALSE, fig.height=5, fig.width=5}
x = runif(100, -3, 3); y = x + sin(x) + rnorm(100, sd = .2); ## sin(x) term gives some                     ## nonlinearity
library(ggplot2)
...
---
## Heteroskedasticity - new data, can't see it here
```{r, echo = FALSE, fig.height=4.5, fig.width=4.5}
x <- runif(100, 0, 6); y <- x + rnorm(100,  mean = 0, sd = .001 * x); ## generated h-y by having
               ## x in the sd term for y
g = ggplot(data.frame(x = x, y = y), aes(x = x, y = y))
g = g + geom_smooth(method = "lm", colour = "black")
g = g + geom_point(size = 7, colour = "black", alpha = 0.4)
g = g + geom_point(size = 5, colour = "red", alpha = 0.4)
...
---
## Residual plot shows the heteroskedasticity
```{r, echo = FALSE, fig.height=4.5, fig.width=4.5}
g = ggplot(data.frame(x = x, y = resid(lm(y ~ x))), 
           aes(x = x, y = y))
g = g + geom_hline(yintercept = 0, size = 2); 
...
fit = factor(c(rep("Itc", nrow(diamond)),  ## label the two new kinds of residuals
               rep("Itc, slope", nrow(diamond))))
g = ggplot(data.frame(e = e, fit = fit), aes(y = e, x = fit, fill = fit))  ## nice code
g = g + geom_dotplot(binaxis = "y", size = 2, stackdir = "center", binwidth = 20)
g = g + xlab("Fitting approach")
g = g + ylab("Residual price")
g     ### this plot shows the variation explained by the regression model and what's left
```
...
fit <- lm(y ~ x)
summary(fit)$sigma
sqrt(sum(resid(fit)^2) / (n - 2))  ## manual calc of sigma
```
