...
require(datasets)
data(swiss)
require(GGally)   ### extension to ggplot2. makes a ggplot version of the pairs plot
require(ggplot2)
...
terms that are linear combinations of other terms. It will show `NA` for
...
### `I` means to do the operation within the lm.
### 1 * ... changes it from Boolean to numeric
```
...
library(dplyr)
summarise(group_by(InsectSprays, spray), mn = mean(count))  ### same numbers as the 
     ## no intercept lm, but the inference is different (vs 0, not vs ref group)
```

---
## Reordering the levels
```{r}
spray2 <- relevel(InsectSprays$spray, "C")    ### define your own reference group
