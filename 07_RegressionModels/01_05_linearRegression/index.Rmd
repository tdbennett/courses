...
```{r, echo = TRUE}
fit2 <- lm(price ~ I(carat - mean(carat)), data = diamond) ## mean-center X
     ## I function Identity, if you want arithmetic statements inside lm calls
coef(fit2)   ### same slope, intercept is now price for average diamond
summary(fit2)
```
...
```{r, echo = TRUE}
fit3 <- lm(price ~ I(carat * 10), data = diamond)  ## per 1/10 carat  ## see slides above
coef(fit3)
```
...
```{r, echo = TRUE}
newx <- c(0.16, 0.27, 0.34)  ### carat sizes of new diamonds
coef(fit)[1] + coef(fit)[2] * newx
predict(fit, newdata = data.frame(carat = newx))  ## useful call, scales up to complex models
predict(fit)  ## gives the y^hat values
```
