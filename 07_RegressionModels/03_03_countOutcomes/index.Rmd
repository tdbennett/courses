...
## Website data - TB the download doesn't run, but the data are in the repo
...
load("07_RegressionModels/03_03_countOutcomes/data/gaData.rda")
gaData$julian <- julian(gaData$date) ### this is another way to transform a date into 1/1/70 stats format
head(gaData)
```
...
- There's a problem with logs with you have zero counts, adding a constant works - frequently + 1
- geometric mean is exp(ln(arithmetic mean))
```{r}
round(exp(coef(lm(I(log(gaData$visits + 1)) ~ gaData$julian))), 5)  ### this does geometric means
```
...
__Poisson/log-linear__  TB - note that Poisson and log-linear are close
...
```{r}
confint(glm1)   ### neither of these are exponentiated
...
## Fitting rates in R - BY a relative term - monitoring time, person-time at risk, etc

The offset term does this - ends up as ln(term +1)  
    the +1 deals with any 0's

```{r ratesFit,dependson="agnostic", cache=TRUE,fig.height=4,fig.width=4}
glm2 <- glm(gaData$simplystats ~ julian(gaData$date), offset=log(visits+1),  ### the hits from SS relative to total
            family="poisson", data=gaData)
plot(julian(gaData$date), glm2$fitted,col="blue",pch=19, xlab="Date", ylab="Fitted Counts")  ### model with offset
points(julian(gaData$date), glm1$fitted,col="red", pch=19)  ### model without offset
```
