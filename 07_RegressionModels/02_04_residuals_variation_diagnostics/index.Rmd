...
* Leverage: distance from center of cloud of data
* Influence: How far from the regression relationship is it? 
...
* Upper right hand point has high leverage, but chooses not to exert it and thus has low influence by conforming to the regression relationship of the other points.
...
* Do `?influence.measures` to see the full suite of influence measures in stats. The measures include
  ...
  * `hatvalues` - measures of leverage (might be useful to catch data entry errors)
...
x <- c(10, rnorm(n))  ### random uncorrelated data plus one at 10,10 - not on the regression line
y <- c(10, c(rnorm(n)))
...
```{r}
fit <- lm(y ~ x)
round(dfbetas(fit)[1 : 10, 2], 3) ### the first point, the 10,10 point, has a huge dfbeta and hatvalue relative to the others
...
x <- rnorm(n)
y <- x + rnorm(n, sd = .3)
x <- c(5, x)  ### outlier is at 5,5 and is on the regression line
y <- c(5, y)
plot(x, y, frame = FALSE, cex = 2, pch = 21, bg = "lightblue", col = "black")
fit2 <- lm(y ~ x)
abline(fit2)            
...
```{r, echo = TRUE}
round(dfbetas(fit2)[1 : 10, 2], 3)  ### dfbeta still large, but not as large (lower influence)
round(hatvalues(fit2)[1 : 10], 3)   ### hatvalue still very big - high leverage, even if low influence
```
...
plot(predict(fit), resid(fit), pch = '.')   ### multivariate model, common to plot residuals vs fitted values, not vs x
```
