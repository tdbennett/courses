...
```{r, echo = TRUE}
n <- 100; nosim <- 1000
x1 <- rnorm(n)   ### unrelated variables
x2 <- rnorm(n)
x3 <- rnorm(n)
...
nosim <- 1000
x1 <- rnorm(n)
x2 <- x1/sqrt(2) + rnorm(n) /sqrt(2)   ### x2 depends on x1
x3 <- x1 * 0.95 + rnorm(n) * sqrt(1 - 0.95^2);  ### x3 depends on x1
betas <- sapply(1 : nosim, function(i){
  y <- x1 + rnorm(n, sd = .3)
  c(coef(lm(y ~ x1))[2], 
    coef(lm(y ~ x1 + x2))[2], 
...
#### TB this is not from the course - found online
require(lmtest)
lrtest(fit1, fit3)
```
