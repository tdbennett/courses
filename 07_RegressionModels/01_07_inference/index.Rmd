...
---
## Example diamond data set - manual CI calcs
```{r}
...
coefTable
fit <- lm(y ~ x); 
summary(fit)$coefficients  ### same table
```
...
---
## Plotting the prediction intervals - they are different from CI's for y
```{r, fig.height=5, fig.width==5, echo = FALSE, results='hide'}
library(ggplot2)
newx = data.frame(x = seq(min(x), max(x), length = 100))
p1 = data.frame(predict(fit, newdata= newx,interval = ("confidence"))) ## confidence in regression line - very narrow
p2 = data.frame(predict(fit, newdata = newx,interval = ("prediction"))) ## what I think of as a CI, CI of est x at a new y
...
* Both intervals have varying widths.
  * Both have least width at the mean of the Xs.
